document.addEventListener("DOMContentLoaded", function() {
	...
	if ( yearInput.value.length === 4 ) {
		if ( currentYear - yearInput.value < 15 ) {
			carAge = currentYear - yearInput.value - 1;
		} else {
			carAge = 15;
		}
	}
	...
	const finGarantyResult = document.querySelector(".result-fin-garanty"); // Фінансова гарантія
	const insuranceDangerousCargoResult = document.querySelector(".result-insurance-dangerous-cargo"); // Страхування небезпечного вантажу
...
function setEngineVolumeTitle() {
	...
	function calculateTaxes() {
		...
		const euroToDollarCoef = euroDollarCoef; // Коефіцієнт множення
		...
		if ( selectedEngineType === 0 || !customsDutyResult.value && selectedEngineType === "electro" || priceLotInput.value === "" ) {
			importTax = 0;
	...
	// Встановлюємо фінансову гарантію
	function calculateFinGaranty() {
		finGarantyResult.textContent = 0;
		if ( selectedEngineType !== "electro" ) {
			finGarantyResult.textContent = 0;
		} else {
			finGarantyResult.textContent = 150;
		}
	}

	// Встановлюємо страхування небезпечного вантажу
	...
	function calculateFullPrice() {
		...
		fullPrice = +priceLotInput.value + 
								...
								+finGarantyResult.textContent +
								+insuranceDangerousCargoResult.textContent;
		let notFullPrice = 0;
	...
	function callAllFunctions() {
		...
		calculateFinGaranty(); // Фін гарантія
